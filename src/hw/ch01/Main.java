...
public class Main {
    public static void main(String[] args) {
        ...
        // 장르가 "소설"인 책만 출력하기
        System.out.println("장르가 소설인 책 출력하기:");
        Iterator<Book> itGenre = bookShelf.iteratorByGenre("소설");
        while (itGenre.hasNext()) {
            Book book = itGenre.next();
            System.out.printf("%-15s | %-3s | %-4d년 | %-5.0f원%n",
                book.getName(), book.getGenre(), book.getYear(), book.getPrice());

        }

        System.out.println("\n최신 책 순으로 모든 책 출력하기:");
        Iterator<Book> itLatest = bookShelf.iteratorByYear();
        while (itLatest.hasNext()) {
            Book book = itLatest.next();
            System.out.printf("%-15s | %-3s | %-4d년 | %-5.0f원%n",
                book.getName(), book.getGenre(), book.getYear(), book.getPrice());
        }
    }
